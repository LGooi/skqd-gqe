...
@dataclass(frozen=True, slots=True)
class SKQDResult:
    samples: tuple[SKQDSampleResult, ...]
    local_refined: SKQDSampleResult | None = None
    global_refined: SKQDSampleResult | None = None
    ...
    @property
    def best_sample(self) -> SKQDSampleResult | None:
        if not self.samples:
            return None
