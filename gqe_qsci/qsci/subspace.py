...
class DeterminantSubspace:
    ...
    def union(self, others: Sequence["DeterminantSubspace"]) -> "DeterminantSubspace":
        seen: set[tuple[int, int]] = set()
        merged: list[Determinant] = []
        for space in (self, *others):
            for det in space.determinants:
                key = (int(det[0]), int(det[1]))
                if key in seen:
                    continue
                seen.add(key)
                merged.append(det)
        return DeterminantSubspace(merged)
